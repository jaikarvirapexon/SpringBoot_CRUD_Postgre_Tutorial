...
Phase numbering used throughout: 1 = Environment check, 2 = Write/load flows, 2b = Stack smoke, 3 = Run all flows, 4 = Update test-case JSON, 5 = Consolidated report.
...
Cross-check every row in PLAN.md `## 5. Task Breakdown` against on-disk artifacts. Catches the silent-skip failure mode where PLAN listed a documentation or doc-section task; implementation skipped it; nobody noticed.
...
Rules:
- `Existence` — `target_file` exists on disk after implementation
- `Section match` — for tasks worded "Add <X> section" / "update <Y> section" / "include <Z>", grep `target_file` for the section heading or cited identifier
...
Each manual TC without a recorded result writes an entry to
