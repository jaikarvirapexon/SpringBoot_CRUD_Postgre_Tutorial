...
- Governance profile: read `harness.yaml governance.profile` (default `standard`). Record active compliance overlays (`hipaa`/`pci`/`sox`/`gdpr`) and apply their rules.
...
## Compliance impact
(only when governance.profile != "standard")
