...
5. **When `design_mode != none`** — hand off to `ux-agent` (per-provider, composer-wired). The ux-agent reads `## Screen inventory`, produces `DESIGN.md`, replaces `## Visual spec` stub with the final one-line pointer, sets `design = complete` in `docs/features/$ARGUMENTS/state.json` (B-tier; mirror to index). Provider-specific procedure lives in:
   - `figma` → `figma-design` skill (Figma MCP, full Phase 1–7 workflow)
   - `claude-design` → `claude-design` skill (manual export consumer; reads `docs/design/<id>/`)
   - `stitch` → `stitch-design` skill (Stitch MCP; scaffold body — team fills before use)
   - `html-mockup` → `html-mockup-design` skill (generates standalone HTML files; no external tool)
6. **When `design_mode == none`** — no ux-agent handoff. `## Visual spec` body becomes `Not applicable — integrations.design = none. Backend / API / data feature.` `design = "n/a"` in `docs/features/$ARGUMENTS/state.json` (B-tier; mirror to index).
