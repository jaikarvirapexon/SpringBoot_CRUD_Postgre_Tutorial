...
| design_mode | Phase 1 behaviour |
|---|---|
| `figma` | After draft, invoke `ux-agent` (figma) → composes hi-fi frames via Figma MCP → writes `DESIGN.md` + sets `## Visual spec` pointer in REQUIREMENTS.md. |
| `claude-design` | After draft, invoke `ux-agent` (claude-design) → ingests user-exported bundle from `docs/design/<id>/` → writes `DESIGN.md`. No MCP; bundle must exist locally. |
| `stitch` | After draft, invoke `ux-agent` (stitch) → generates screens via Stitch MCP → writes `DESIGN.md`. Procedure body still scaffold; team fills `stitch-design` skill before first use. |
| `html-mockup` | After draft, invoke `ux-agent` (html-mockup) → generates standalone HTML files per screen × form factor → writes `DESIGN.md`. No external tool, no MCP. |
| `none` | Skip `## Visual spec` + `## Screen inventory` in PRD. Set `design = "n/a"` (writes to `docs/features/$ARGUMENTS/state.json` at the migration point in Step 1). |

In every non-`none` mode the visual spec lives in `docs/features/<id>/DESIGN.md`. The PRD's `## Visual spec` section is a one-line pointer only — never inline screens / tokens / wireframes.
