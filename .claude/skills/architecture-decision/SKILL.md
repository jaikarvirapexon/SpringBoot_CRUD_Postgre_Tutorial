...
**DO NOT decide here** (out of scope — belongs to `/arh-plan-implementation` Phase 1 mini-ADRs):
...
- Output: ONE consolidated ADR. `Status: Accepted` for decisions the user confirmed; `Status: Proposed` / `[NEEDS CLARIFICATION]` for any the user deferred — do not fill a gap by guessing. Include `Alternatives considered` (the recommendations not taken) and `Consequences` incl. reversibility.
