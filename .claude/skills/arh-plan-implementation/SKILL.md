...
Invoke the `impl-planning-agent` subagent with `$ARGUMENTS`. It applies skill `plan-authoring`: mini-ADRs (+ structured `decide` entries), file + module plan with `F-NN` ids (every new module's entry-registration site listed as edited), `T-NN` task breakdown with carry-forward link-through to the research risk register, **self-validation via the `plan-validation` rubric** (4 dimensions: wiring / docs / runner-setup / cross-section; ≤2 self-correct rounds, then escalation), the test strategy, and writes `docs/features/$ARGUMENTS/PLAN.md` plus the `plan_validation` state fields.
