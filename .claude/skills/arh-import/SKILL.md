...
argument-hint: "[--jira-jql Q | --linear-team T | --confluence-space S | --notion-database ID | --from-files PATH] [--force] [--priority-map PATH]"
...
2. Pulls items via the configured MCP server (atlassian, linear, notion, confluence).
