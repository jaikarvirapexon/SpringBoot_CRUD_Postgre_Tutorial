...
- Batch up to 50 issues per MCP call when the provider supports it (Jira `searchByJQL`, Linear bulk).
...
- **MCP auth failure** — abort Phase 1 entirely with the env var name to set; do not partially apply.
...
| `X` | `X` | `Y` | remote-only change — auto-apply when `--apply` |
