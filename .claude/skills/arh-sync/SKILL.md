...
Hybrid flow: the gate (Phase 0) runs inline here in the main session; the pull + merge + report (Phase 1) is delegated to the `sync-agent` subagent (model `haiku`), which applies the `tracker-sync` skill plus the provider integration skills its frontmatter declares (via Jinja, per the project's `harness.yaml integrations`): `requirement-tracing`, one of `issue-tracking-jira` / `issue-tracking-linear` / `issue-tracking-github` / `issue-tracking-azure`, and one of `doc-tracker-confluence` / `doc-tracker-notion` when configured.
