...
// invocation into docs/activity/<YYYY-MM>.jsonl. Reads the session transcript
...
{
  const startTs = events[commandIndex]?.timestamp || events[commandIndex]?.message?.timestamp;
  const endTs = events[events.length - 1]?.timestamp || events[events.length - 1]?.message?.timestamp;
  if (startTs && endTs) {
...
// Month file derived from the invocation timestamp (not now) so every fire for
// one invocation lands in the same file even across a month boundary.
function monthOf(iso) {
  const d = new Date(iso);
  const base = Number.isNaN(d.getTime()) ? new Date() : d;
  return `${base.getUTCFullYear()}-${String(base.getUTCMonth() + 1).padStart(2, '0')}`;
}
const outPath = resolve(projectDir, `docs/activity/${monthOf(cmdTs)}.jsonl`);
